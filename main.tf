...
# GCP and GKE modules are generated by terragrunt into gcp_modules.tf
# when has_gcp = true. See terragrunt.hcl.
...
locals {
  cluster_control_plane_ips = {
    for cluster_name, cluster in var.clusters : cluster_name => merge(
      ...
      try(module.gcp[cluster_name].control_plane_ips, {}),
      lookup(local.gke_clusters, cluster_name, null) != null ? { "${cluster_name}-gke" = try(module.gke[cluster_name].cluster_endpoint, "") } : {}
    )
  }
  ...
  cluster_worker_ips = {
    for cluster_name, cluster in var.clusters : cluster_name => merge(
      ...
      try(module.gcp[cluster_name].worker_ips, {})
    )
  }
  ...
  cluster_install_complete = {
    for cluster_name, cluster in var.clusters : cluster_name => concat(
      ...
      try(module.gcp[cluster_name].install_complete, []),
      lookup(local.gke_clusters, cluster_name, null) != null ? try(module.gke[cluster_name].install_complete, []) : []
    )
  }
  ...
  all_kubeconfigs = merge(
    ...
    { for k, v in local.gke_clusters : k => try(module.gke[k].kubeconfig, "") }
  )
