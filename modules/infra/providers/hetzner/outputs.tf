...
output "control_plane_internal_ips" {
  value = {
    for cp in hcloud_server.control_plane :
    cp.labels["node-name"] => one(cp.network).ip
  }
}
...
output "worker_internal_ips" {
  value = {
    for w in hcloud_server.worker :
    w.labels["node-name"] => one(w.network).ip
  }
}
...
output "ceph_disk_ids" {
  value = local.ceph_enabled ? [for v in hcloud_volume.ceph_osd : v.id] : []
}
