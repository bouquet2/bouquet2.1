...
variable "hetzner_network_cidr" {
  type        = string
  description = "Hetzner private network CIDR used to assign deterministic per-node private IPs"
}
...
variable "ceph_disk" {
  type = object({
    ...
    include_control_planes = optional(bool, false)
  })
  default     = {}
  description = "Ceph OSD disk configuration. Size in GB."
}
