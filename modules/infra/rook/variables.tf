...
variable "osd_devices" {
  type        = map(string)
  default     = {}
  description = "Provider-specific device path per node. When specified, uses explicit devices instead of auto-discovered ones."
}

variable "device_filter" {
  type        = string
  default     = "^/dev/disk/by-id/scsi-0Google_PersistentDisk"
  description = "Regex filter for auto-discovering devices. Used when osd_devices is empty."
}
