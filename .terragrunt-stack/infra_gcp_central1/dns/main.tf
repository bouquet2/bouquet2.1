...
resource "cloudflare_dns_record" "lb_cluster" {
  for_each = local.all_node_ips

  zone_id = data.cloudflare_zone.this.zone_id
  name    = "${var.lb_subdomain}.${var.cluster_names[0]}"
  type    = "A"
  content = each.value
  ttl     = 1
  proxied = true
...
resource "cloudflare_dns_record" "control_plane_public_cluster" {
  for_each = local.all_control_plane_ips

  zone_id = data.cloudflare_zone.this.zone_id
  name    = "control-planes.${var.cluster_names[0]}"
  type    = "A"
  content = each.value
  ttl     = 1
  proxied = true
...
resource "cloudflare_dns_record" "worker_public_cluster" {
  for_each = local.all_worker_ips

  zone_id = data.cloudflare_zone.this.zone_id
  name    = "workers.${var.cluster_names[0]}"
  type    = "A"
  content = each.value
  ttl     = 1
  proxied = true
...
resource "cloudflare_dns_record" "lb_global" {
  for_each = var.primary_cluster == var.cluster_names[0] ? local.all_node_ips : {}
